...
function buildFavList(response){
    for (var i = 0; i < response.length; i++) {
        ...
        itemHTML += '<button class="controlButtonsP playIco" onclick="playAudio(response[i].sound)" alt="Play"><i class="fas fa-play"></i></button></div>'
        itemHTML += '<button class="controlButtonsP stopIco" onclick="stopAudio(response[i].sound)" alt="Stop"><i class="fas fa-stop"></i></button></div>'
